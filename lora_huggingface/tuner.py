...
from metaflow_callback import MetaflowCheckpointCallback
...
def train(config_file) -> None:
    ...
    )  # the only required argument # TrainingParams.base_model
    assert (
        config.model.base_model
    ), "Please specify a model.base_model, e.g. 'huggyllama/llama-7b'"
    if neither_is_none_or_both_are_none(
        config.dataset.local_dataset_path, config.dataset.huggingface_dataset_path
    ):
        raise ValueError(
    ...
    gradient_accumulation_steps = (
        config.training.macro_batch_size // config.training.micro_batch_size
    )
    ...
    if resuming_checkpoint_path is None:
        resuming_checkpoint_path = False
    ...
    trainer = transformers.Trainer(
        ...
        args=transformers.TrainingArguments(
            ...
            save_total_limit=config.training.max_checkpoints,
