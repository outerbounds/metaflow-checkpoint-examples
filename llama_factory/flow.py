from metaflow import FlowSpec, step, kubernetes, secrets, model, current
...
class LlamaFinetuneGridSearch(FlowSpec):
    ...
    def tune(self):
        ...
        json.dump(self.hyperparameters, open('train_cfg.json', 'w', encoding='utf-8'), indent=2)
        proc = subprocess.run(['llamafactory-cli', 'train', 'train_cfg.json'])
        if proc.returncode != 0:
            print(f"Training subprocess failed with return code {proc.returncode}. Exiting.")
            sys.exit(proc.returncode)
        self.trained_model_info = current.model.save(self.output_dir, metadata=self.hyperparameters)
        self.next(self.join)
